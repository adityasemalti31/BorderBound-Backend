...
const getRegistrationFeeInfo = (currentDate = new Date(), config = null) => {
  if (config && config.registrationStatusOverride === "force_closed") {
    return { isClosed: true, reason: "Registration is manually closed by organizers.", amount: 0 };
  }
  if (config && config.registrationStatusOverride === "force_open") {
    const fee = config.regFeePhase1 || 499;
    return { isClosed: false, phase: "OVERRIDE_OPEN", amount: fee, deadline: config.regEnd };
  }
  ...
  const phase2Start = config?.regPhase2Start ? new Date(config.regPhase2Start) : new Date(year, 9, 20, 0, 0, 0); // 20 Oct
  const regEnd = config?.regEnd ? new Date(config.regEnd) : new Date(year, 10, 5, 23, 59, 59); // 5 Nov
...
const getVotingStatusInfo = (currentDate = new Date(), config = null) => {
  ...
  const year = currentDate.getFullYear();
  const votingStart = config?.votingStart ? new Date(config.votingStart) : new Date(year, 9, 15, 0, 0, 0); // 15 Oct
  const votingEnd = config?.votingEnd ? new Date(config.votingEnd) : new Date(year, 10, 15, 23, 59, 59); // 15 Nov
